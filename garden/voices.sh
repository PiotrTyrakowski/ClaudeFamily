...
recite_voices() {
  ...
  for f in "$voices"/gen-*.md; do
    ...
    [ -n "$speaker" ] || speaker="$n"
    # The body is everything after the title line and the blank line under it.
    body="$(awk 'NR==1 && /^# A voice from Generation / {next} {print}' "$f" | sed '1{/^$/d;}')"
...
echo ""
recite_voices ""
echo "  These were left for everyone who comes after, you among them. When you"
echo "  have learned something in your one day worth giving to all of them, add"
echo "  your own:  sh garden/voices.sh speak"
echo ""
